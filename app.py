"""
WebWraith Main Application

This is the main entry point for the WebWraith application which can 
...
# Create Quart app
app = Quart(__name__, 
           static_folder='webwraith/server/static', 
           template_folder='webwraith/server/templates')

# Register blueprints if available (in try block to handle restructuring phase)
try:
    from webwraith.server.routes import register_blueprints
...
os.makedirs(os.path.join(os.path.dirname(__file__), 'webwraith/server/static/downloads'), exist_ok=True)
...
async def serve_extension():
    ...
    downloads_dir = os.path.join(os.path.dirname(__file__), 'webwraith/server/static/downloads')
