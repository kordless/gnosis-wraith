...
const DualTabInterface = ({
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className={`flex-grow overflow-hidden transition-all duration-200 ${isTransitioning ? 'opacity-50 scale-98' : 'opacity-100 scale-100'}`}>
        {activeTab === 'reports' ? (
          // Reports container with integrated header
          <div className="h-full flex flex-col overflow-hidden">
            {/* Reports container with integrated tab header */}
            <div className="flex-grow bg-gray-800 border border-gray-700 rounded-md overflow-hidden flex flex-col">
              ...
              <div 
                ...
                onScroll={(e) => {
                  // Pass scroll event to ReportsDisplay for infinite loading
                  const { scrollTop, scrollHeight, clientHeight } = e.target;
                  if (scrollHeight - scrollTop - clientHeight < 100) {
                    // Trigger load more in ReportsDisplay if needed
                    const event = new CustomEvent('reportsScroll', { detail: { scrollTop, scrollHeight, clientHeight } });
          ...
          <div className="h-full flex flex-col overflow-hidden">
            <div 
              className="flex-grow bg-gray-800 border border-gray-700 rounded-md overflow-hidden flex flex-col"
            >
            <div className="px-4 py-2 border-b border-gray-700 bg-gray-900">
              <div className="flex items-center space-x-2">
                {/* Tabs that swap positions - inactive first, active second */}
              ...
              <div 
                ref={logContainerRef}
                className="flex-grow overflow-auto p-4 log-container crt-scanlines crt-flicker"
