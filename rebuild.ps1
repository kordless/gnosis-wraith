...
$faviconDir = Join-Path -Path $PSScriptRoot -ChildPath "gnosis_wraith\server\static\images"
...
Log-Message "  Using Dockerfile.jobs"
...
Log-Message "  Command: docker build --build-arg EXTENSION_VERSION=$manifestVersion -t gnosis-wraith -f Dockerfile.jobs ." -ForegroundColor Gray
Log-Message "  Build output will be logged to $logFilePath" -ForegroundColor Gray

# Execute the build and capture output
$buildOutput = & docker build --build-arg EXTENSION_VERSION=$manifestVersion -t gnosis-wraith -f Dockerfile.jobs . 2>&1
